    ...
    <div class="card-body row">
      ...
      <div class="col-sm-6">
        <div class="form-group">
          ...
          <select class="form-control select2bs4" name="driver_id" required>
            ...
            @foreach ($crew_list as $crew)
              <option value="{{ $crew->id }}" {{ old('driver_id') == $crew->id ? 'selected' : '' }}>
        ...
        <div class="form-group">
          ...
          <select class="form-control select2bs4" name="bus_id" required>
            ...
            @foreach ($bus_list as $bus)
              <option value="{{ $bus->uuid }}" {{ old('bus_id') == $bus->uuid ? 'selected' : '' }}>
                {{ $bus->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="form-group">
          ...
          <input type="number" class="form-control" name="work_day_count" value="{{ old('work_day_count', 0) }}" min="0" required>
        </div>

        <div class="form-group">
          <label>Jam Istirahat (12 jam terakhir) <span class="text-danger">*</span></label>
          <input type="number" class="form-control" name="rest_hours_last_12h" value="{{ old('rest_hours_last_12h', 0) }}" min="0" step="0.5" required>
        </div>
      </div>
      ...
      <div class="col-sm-6">
        <div class="form-group">
          ...
          <input type="number" class="form-control" name="blood_pressure_systolic" value="{{ old('blood_pressure_systolic') }}" placeholder="mmHg" required>
        </div>

        <div class="form-group">
          <label>Tekanan Darah Diastolik <span class="text-danger">*</span></label>
          <input type="number" class="form-control" name="blood_pressure_diastolic" value="{{ old('blood_pressure_diastolic') }}" placeholder="mmHg" required>
        </div>

        <div class="form-group">
          <label>Suhu Tubuh (&deg;C) <span class="text-danger">*</span></label>
          <input type="number" class="form-control" name="body_temperature" value="{{ old('body_temperature') }}" step="0.1" placeholder="Contoh: 36.5" required>
        </div>

        <div class="form-group">
          <label>Status Denyut Jantung <span class="text-danger">*</span></label>
          <select class="form-control" name="heart_rate_status" required>
            <option value="normal" {{ old('heart_rate_status') == 'normal' ? 'selected' : '' }}>Normal</option>
            <option value="abnormal" {{ old('heart_rate_status') == 'abnormal' ? 'selected' : '' }}>Abnormal</option>
          </select>
        </div>
